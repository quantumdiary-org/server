...
// CacheItem - элемент кэша для внутреннего использования
...
func (m *MemoryCache) Get(ctx context.Context, key string, target interface{}) (bool, error) {
	...
	if !exists || item.IsExpired() {
		delete(m.data, key) // Удаляем просроченные элементы
		return false, nil
	}
...
// CacheWithFallback предоставляет стратегию кэширования с fallback
...
func (c *CacheWithFallback) Get(ctx context.Context, key string, target interface{}) (bool, error) {
	...
	// Если нет в основном кэше или ошибка, пробуем fallback
